...
class BshbOpenDoorWindowHandler extends bshb_handler_1.BshbHandler {
    ...
    handleBshcUpdate(resultEntry) {
        ...
        const condition1 = resultEntry["@type"] === "DeviceServiceData" &&
            resultEntry.id === "ShutterContact";
        // setting of device changed. Probably the profile. So we refresh
        const condition2 = resultEntry["@type"] === "device" &&
        ...
        if (condition1 || condition2) {
            ...
            this.detectOpenDoorsAndWindows().subscribe(this.handleBshcUpdateError(`condition1=${condition1}, condition2=${condition2}`));
            return true;
        }
        return false;
    }
    ...
    detectOpenDoorsAndWindows() {
        return this.getBshcClient()
            ...
            .pipe((0, rxjs_1.tap)(() => this.setObjectNotExistsAsync("openDoorsAndWindows", {
            type: "folder",
            common: {
    ...
    getElements(result, key) {
        if (key.startsWith("--all")) {
            ...
            if (typeof result.parsedResponse[key] === "undefined" ||
                result.parsedResponse[key] === null) {
