...
class BshbDeviceHandler extends bshb_handler_1.BshbHandler {
    ...
    handleDetection() {
        ...
        const devices = this.detectDevices().pipe((0, rxjs_1.catchError)(err => {
            // skip errors here
            this.bshb.log.warn('Failure during detection in BshbDeviceHandler. Continue with cached data. This only works if the adapter has been started successfully at least once. New devices may not be recognized. ' +
                err);
    ...
    sendUpdateToBshc(id, state) {
        ...
        if (cachedState &&
            ...
            result = this.mapValueFromStorage(id, state.val).pipe((0, rxjs_1.switchMap)(value => {
            ...
            }), (0, rxjs_1.tap)(this.handleBshcSendError(`path=${cachedState.deviceService.path}`)), (0, rxjs_1.map)(() => true));
        }
        return result;
    }
    restoreCache() {
        const start = (0, rxjs_1.of)('').pipe((0, rxjs_1.tap)(() => this.bshb.log.info('Restoring cache started...')));
        const preparation = start.pipe((0, rxjs_1.switchMap)(() => this.setObjectNotExistsAsync('info.cache', {
            type: 'folder',
            common: {
                name: 'cache',
            },
            native: {},
        })), (0, rxjs_1.switchMap)(() => this.setObjectNotExistsAsync('info.cache.rooms', {
            type: 'state',
            common: {
                name: 'rooms',
                type: 'object',
        ...
        // Restore rooms cache
        (0, rxjs_1.tap)(() => this.bshb.log.info('Restoring cache: rooms')), (0, rxjs_1.switchMap)(() => (0, rxjs_1.from)(this.bshb.getStateAsync('info.cache.rooms'))), (0, rxjs_1.filter)(this.isDefined), (0, rxjs_1.switchMap)(roomState => this.mapValueFromStorage('info.cache.rooms', roomState.val)), (0, rxjs_1.tap)(roomState => {
            for (const [key, value] of Object.entries(roomState)) {
                this.bshb.log.debug('Restore cache room: ' + key);
                this.cachedRooms.set(key, value);
            }
        }));
        const devices = rooms.pipe((0, rxjs_1.tap)(() => this.bshb.log.info('Restoring cache: devices, device service and states')), 
        // Restore devices, services and states
        // First get all objects and create stream
        (0, rxjs_1.switchMap)(() => (0, rxjs_1.from)(this.bshb.getDevicesAsync())), (0, rxjs_1.switchMap)(result => (0, rxjs_1.from)(result)), (0, rxjs_1.filter)(obj => obj.native.device && obj.native.device.id), 
        // Restore device
        (0, rxjs_1.tap)(obj => {
        ...
        (0, rxjs_1.mergeMap)(obj => (0, rxjs_1.from)(this.bshb.getChannelsOfAsync(obj.native.device.id))), (0, rxjs_1.switchMap)(result => (0, rxjs_1.from)(result)), (0, rxjs_1.filter)(obj => obj.native.device &&
        ...
        const states = deviceServices.pipe((0, rxjs_1.mergeMap)(obj => (0, rxjs_1.from)(this.bshb.getStatesOfAsync(obj.native.device.id, obj.native.deviceService.id))), (0, rxjs_1.switchMap)(result => (0, rxjs_1.from)(result)), (0, rxjs_1.filter)(obj => obj.native.device &&
            obj.native.device.id &&
            obj.native.deviceService &&
            obj.native.deviceService.id &&
            obj.native.state), (0, rxjs_1.tap)(stateObj => {
        ...
        return states.pipe((0, rxjs_1.tap)({
            complete: () => this.bshb.log.info('Restoring cache finished'),
        }), (0, rxjs_1.switchMap)(() => (0, rxjs_1.of)(undefined)), (0, rxjs_1.catchError)(err => {
    ...
    detectDevices() {
        const rooms = this.getBshcClient()
            ...
            .pipe((0, rxjs_1.tap)(() => this.bshb.log.info('Start detecting devices...')), (0, rxjs_1.switchMap)(response => {
            this.bshb.log.debug(`Found ${response.parsedResponse.length} rooms.`);
            return (0, rxjs_1.from)(response.parsedResponse);
        }), (0, rxjs_1.switchMap)(room => {
        ...
        const devices = this.getBshcClient()
            ...
            .pipe((0, rxjs_1.switchMap)(response => (0, rxjs_1.from)(response.parsedResponse)), (0, rxjs_1.mergeMap)(device => {
            ...
            return this.setObjectNotExistsAsync(device.id, {
            ...
            }).pipe((0, rxjs_1.tap)(obj => {
                if (obj && obj._bshbCreated) {
                    this.addRoom(device.id, '', undefined, device.roomId);
                }
            }), (0, rxjs_1.switchMap)(() => this.setObjectNotExistsAsync(deviceStatusId, {
                type: 'state',
                common: {
                    name: 'status',
                    type: 'string',
                ...
                native: {},
            })), (0, rxjs_1.switchMap)(() => (0, rxjs_1.from)(this.bshb.getStateAsync(deviceStatusId))), (0, rxjs_1.switchMap)(state => this.setInitialStateValueIfNotSet(deviceStatusId, state, device.status)), (0, rxjs_1.switchMap)(() => {
        ...
        return (0, rxjs_1.concat)(rooms, devices, this.checkDeviceServices()).pipe((0, rxjs_1.tap)({
    ...
    checkDeviceServices() {
        return this.getBshcClient()
            ...
            return (0, rxjs_1.from)(this.bshb.getObjectAsync(deviceService.deviceId)).pipe((0, rxjs_1.switchMap)(ioBrokerDevice => {
                if (!ioBrokerDevice) {
    ...
    importChannels(device, deviceService) {
        ...
        // add fault holder
        (0, rxjs_1.switchMap)(() => this.importSimpleState(id, device, deviceService, 'faults', BshbDeviceHandler.getFaults(deviceService), false)), (0, rxjs_1.tap)(() => this.cachedDeviceServices.set(deviceService.path, {
            device: device,
            deviceService: deviceService,
        })), (0, rxjs_1.switchMap)(() => this.importStates(id, device, deviceService)));
    }
    ...
    importSimpleState(idPrefix, device, deviceService, stateKey, stateValue, write) {
        ...
        }).pipe((0, rxjs_1.switchMap)(() => (0, rxjs_1.from)(this.bshb.getStateAsync(id))), (0, rxjs_1.switchMap)(state => this.setInitialStateValueIfNotSet(id, state, stateValue)));
    }
    ...
    addFunction(deviceId, deviceServiceId, itemId) {
        const name = bshb_definition_1.BshbDefinition.determineFunction(deviceServiceId);
