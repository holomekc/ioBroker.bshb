...
export class BshbBackupHandler extends BshbHandler {
    ...
    handleDetection(): Observable<void> {
        return this.detectBackup().pipe(tap({
    ...
    private storeBackupFile(response: BshbResponse<BinaryResponse>) {
        ...
        return from(this.ensureDirectoryExists(backupDir))
            .pipe(
                switchMap(() => {
    ...
    private detectBackup() {
        return this.createBackupFolder()
            .pipe(
                ...
                switchMap(() => this.getBshcClient().getBackupStatus({timeout: this.long_timeout})
                    .pipe(
                ...
                switchMap(() => this.getBshcClient().getRestoreStatus({timeout: this.long_timeout})
                    .pipe(
                        mergeMap(response => from(Object.entries<[string, any]>(response.parsedResponse))),
                        filter(([key, _]) => key !== '@type'),
    ...
    private addState(prefix: string, type: string, key: string, value: any) {
        ...
        return this.setObjectNotExistsAsync(id, {
            ...
            tap(() => this.bshb.setState(id, {val: value, ack: true}))
        );
    }
