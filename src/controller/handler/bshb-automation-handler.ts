...
export class BshbAutomationHandler extends BshbHandler {
    ...
    public handleDetection(): Observable<void> {
        return this.detectAutomations().pipe(tap({
            ...
            if (key === 'trigger') {
                result = this.getBshcClient().triggerAutomation(automationId, {timeout: this.long_timeout})
                    .pipe(
                        ...
                        tap(this.handleBshcSendError(`id=${automationId}, value=${state.val}, key=${key}, automationId=${automationId}`)),
                        map(() => true)
                    );
            } else {
                ...
                result = from(this.addAutomationValue(idPrefix, 'enabled', data)
                    ...
                    .pipe(
                        switchMap(() => this.getBshcClient().setAutomation(automationId, data, {timeout: this.long_timeout})),
    ...
    private detectAutomations(): Observable<void> {
        return this.setObjectNotExistsAsync('automations', {
            ...
            switchMap(() => this.getBshcClient().getAutomations(undefined, {timeout: this.long_timeout})),
            switchMap(response =>
            ...
            mergeMap(automation => {
                ...
                return from(this.bshb.setObjectAsync(id, {
                    ...
                    tap(() => this.bshb.setState(`${id}.automationConditions`, {
                    ...
                    tap(() => this.bshb.setState(`${id}.automationTriggers`, {
                    ...
                    tap(() => this.bshb.setState(`${id}.automationActions`, {
                    ...
                    tap(() => this.bshb.setState(`${id}.conditionLogicalOp`, {
