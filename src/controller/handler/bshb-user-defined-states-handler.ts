...
export class BshbUserDefinedStatesHandler extends BshbHandler {
  ...
  public handleDetection(): Observable<void> {
    return this.detectUserDefinedStates().pipe(
      tap({
        subscribe: () =>
          this.bshb.log.info("Start detecting user defined states..."),
        finalize: () =>
          this.bshb.log.info("Detecting user defined states finished"),
      }),
    );
  }
  ...
  public handleBshcUpdate(resultEntry: any): boolean {
    if (resultEntry["@type"] === "userDefinedState") {
      this.bshb.log.debug(
        `Received updated for user defined state id=${resultEntry["id"]} and value=${resultEntry["state"]}`,
      );

      const id = `userDefinedStates.${resultEntry["id"]}`;
      from(
        this.bshb.setState(id, { val: resultEntry["state"], ack: true }),
  ...
  public sendUpdateToBshc(
    ...
    if (match) {
      this.bshb.log.debug(
        `Found user defined state with id=${match[1]} and value=${state.val}`,
      );
      result = this.getBshcClient()
        .setUserDefinedState(match[1], state.val as boolean, {
  ...
  private detectUserDefinedStates(): Observable<void> {
    return this.setObjectNotExistsAsync("userDefinedStates", {
      ...
      switchMap(() =>
        this.getBshcClient().getUserDefinedStates(undefined, {
          timeout: this.long_timeout,
        }),
      ),
      switchMap((response) =>
      ...
      mergeMap((userDefinedState) => {
        this.bshb.log.debug(
          `Found user defined state ${userDefinedState.id}, ${userDefinedState.name}`,
        );
        ...
        return from(
          ...
          tap(() =>
            this.bshb.setState(id, { val: userDefinedState.state, ack: true }),
          ),
        );
      }),
  ...
  private deleteMissingUserDefinedStates(
    userDefinedStates: any[],
  ): Observable<void> {
    return from(this.bshb.getStatesOfAsync("userDefinedStates", "")).pipe(
      ...
      switchMap((object) => {
        ...
        if (!found) {
          return from(
            this.bshb.delObjectAsync(`userDefinedStates.${object.native.id}`),
          ).pipe(
            tap(() =>
              this.bshb.log.info(
            ...
            catchError((err) => {
              this.bshb.log.error(
                `Could not delete user defined state with id=${object.native.id} because: ` +
                  err,
              );
              return of(undefined);
            }),
