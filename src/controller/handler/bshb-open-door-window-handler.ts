...
export class BshbOpenDoorWindowHandler extends BshbHandler {
  ...
  public handleBshcUpdate(resultEntry: any): boolean {
    ...
    const condition1 =
      resultEntry["@type"] === "DeviceServiceData" &&
    ...
    return this.getBshcClient()
      ...
      .pipe(
        ...
        switchMap((result) => {
          ...
          observables.push(
            this.setList(idPrefix, "all.all", "--all--", "All", result),
          );
          observables.push(
            this.setList(
              idPrefix,
  ...
  private getElements(result: BshbResponse<any>, key: string): any[] {
    if (key.startsWith("--all")) {
      ...
      if (
        typeof result.parsedResponse[key] === "undefined" ||
        result.parsedResponse[key] === null
      ) {
        this.bshb.log.error(
          `Could not find open windows/doors with key=${key}`,
        );
        return [];
      } else {
